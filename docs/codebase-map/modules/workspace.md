...
source_hash: d0e7ad0172ed90a00f68a6bbec583e435d1773a306155bef73d65b0b185c1482
source_files: 62
...
`.worktreeinclude`, so the mapping script does not include it in this module's 62-file inventory or
...
- `open/test-fixtures/README.md`: Defines the immutable versioned fixture layout, strict schema 1
  ...
  video and synchronized multichannel audio baselines' exact reproduction and consumer contracts.
...
The mapping inventory contains 58 UTF-8 text artifacts and four binary payloads. Binary media is
...
Media-I/O contracts validate the video catalog's complete matrix and public frame construction, and
open each audio file through the production PCM source to prove exact timing, routing,
synchronization, samples, and continuity. The UTF-8 fixture remains the validator's smallest policy
self-test.
...
- `open/test-fixtures/README.md` defines
  ...
  synchronized multichannel audio reproduction path. The UTF-8 fixture's payload is 6 bytes and its SHA-256 is
...
- Fresh fixture validation reports three valid fixture versions and six payloads. The video fixture
  contains 207 cases in a 13,419-byte binary and is protected by catalog, generator, CLI,
  media-consumer, and canonical-root contracts. The audio fixture contains three 100 ms PCM16 WAVE
  cases and is protected by byte-for-byte generator, CLI, production PCM consumer, and canonical-root
  contracts.
...
The synchronized remote revision now ends at `036149c0a5df6901553b7bce5e34f4c323e1c240`.
...
`036149c0a5df6901553b7bce5e34f4c323e1c240` when the map was refreshed. Its hash describes the exact
62 discovered source files, including four generated binary payloads, layered on that revision.
