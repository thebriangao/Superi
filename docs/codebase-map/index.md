...
transaction owner. Exact desktop document and generated editor project, root timeline, revision,
depths, and normalized next mutation categories drive always-visible workspace controls, action-specific
...
same-document counts, and no depth, mutation category, or history snapshot enters schema 4 or
project persistence.

The desktop project lifecycle now derives one semantic dirty bit by comparing the current public
project hash with the latest explicit save baseline. Schema 2 private continuity retains that
baseline beside the active path, schema 1 reopens migrate cleanly, and equal authored meaning after
undo clears the modified state despite a newer revision or retained redo history. Native titles,
Save availability, replacement, project close, window close, and quit consume the derived bit;
busy or incoherent state fails closed, required saves precede the transition, and save failure
cancels it. Dirty state never enters project schema, engine history, or workspace persistence.
...
platform clipboard roles, projects a basename-only document title with an explicit modified marker,
enables Save only for an idle dirty document, and persists schema 4 route-local docks, ordered tabs,
bounded sizes, visibility, focus, configurable shortcuts, and bounded local background receipts with
schema 1 through 3 migration,
...
before live menu replacement and history remains absent from schema 4 persistence. Its layout header acknowledges native
...
atomically retains its active path, saved semantic baseline, and bounded recents between launches,
revalidates the active project through `LocalProjectHost`, and never persists the engine's
session-only undo or redo stack.

Operating-system project opens use that same policy. Native startup arguments and macOS resource
events enter a bounded process-local queue with monotonic identities rather than executing an Open
transition. The primary React shell installs its listener before reading retained requests, waits
for coherent project and editor state, routes each path through safe replacement, and explicitly
acknowledges it. This preserves missed-event and reload recovery while preventing auxiliary windows
or native association callbacks from bypassing dirty, history, busy, or save-failure handling.
...
unchanged accelerators, strict rejection, and schema 4 history exclusion.
