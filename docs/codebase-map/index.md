...
- `superi-timeline`: OTIO reading and writing, deterministic graph compilation, advanced trim,
  retime, grouped-source, and higher-level edit operations, undo ownership, multicam, persistence,
  and production consumers beyond its foundational native model, authoritative edit state, marker
  and metadata state, exact snapping, primary edit batches, and contract tests.
...
- Native editorial objects, typed track semantics, exact timing, selection, track targeting, sync
  ...
  foundational insert, overwrite, append, replace, lift, and extract operations:
  `superi-timeline`.
