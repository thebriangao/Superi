...
14. The same Tauri project owner persists media organization, source recovery, proxy intent, and
    source-bound content analysis beside stable imported identities. Revision-fenced native search
    composes metadata, editable transcript timing and speaker state, timeline plus clip relationships,
    and local AI content entries into bounded deterministic evidence. React applies bin and saved
    collection scope, rejects late query revisions, and edits the ordinary artifacts without taking
    persistence, search-ranking, source-identity, model-runtime, or network authority.
