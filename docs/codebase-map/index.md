...
platform clipboard roles, projects a basename-only document title, persists workspace presentation,
...
- Native desktop shell state remains presentation-only. Workspace layout and bounded recent paths
  may persist privately, but project bytes, durable document identity, authored state, and engine
...
duplicate close suppression, one-shot resolution, private workspace persistence, real-host
active project restoration, and missing-document degradation with retained recents. Strict
TypeScript, the production frontend build and contract set, focused native integration tests, and
...
unclean-session detection, exact route, panel, project, revision, and lifecycle continuity, orderly
...
- Desktop crash evidence, active-session detection, bounded safe projection, and cross-session route,
  panel, and project intent: `workspace`, which routes engine recovery to the existing lifecycle owner
