...
and project owner while authored replacement events preserve one global order. Native GPU viewer
surfaces remain attached only to the primary webview until the presenter owns an explicit
multi-webview surface model.
...
4. Tauri manages the lifecycle owner, fixed-capacity `EngineConnection`, and strict persistent
   `DesktopWindowState`. It exposes lifecycle, window-session, and generated API commands, restores
   primary and auxiliary workspace webviews before steady state, and joins the window persistence
   and engine process owners after the native host stops. Primary exit requests record orderly
   shutdown without blocking the main thread.
...
7. React renders the serialized application and engine phases, pending acknowledgement, safe failure,
   ...
   independently renders exact persistent window, monitor, focus, fullscreen, previous placement,
