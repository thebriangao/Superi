...
| `superi-color` | [module map](modules/superi-color.md) | `open/crates/superi-color` | Versioned configuration, project working spaces, color math, CPU input and output transforms, GPU wide-gamut, native display transforms, and deterministic viewer analysis, tone mapping, legal-range RGB encoding, LUTs, ICC discovery, and presentation profile guards | Substantial but partial: project-pinned configuration, CPU transforms, managed GPU wide-gamut and eight-mode sRGB native display transforms, an engine CPU display consumer, and a desktop GPU display consumer are implemented; engine export validates a caller-owned delivery stage but does not execute this crate, while ICC evaluation and concrete export conversion remain absent |
...
display outside both authored state and native placement ownership.
...
11. The same `ApplicationProvider` retains one last-valid `EditorStateSnapshot` for presentation,
    ...
    role-aware source state, physical playback drops, scheduling, failure, and canonical editorial
    intent from that same snapshot and retained source monitor without adding a command owner.
...
- `superi-color`: broader config-persisted rule graphs, ICC transform evaluation, HDR and
  non-sRGB GPU output conversion, decoded-render binding to the production viewport, and concrete
  export integration.
