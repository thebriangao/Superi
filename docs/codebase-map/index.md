...
| `superi-api` | [module map](modules/superi-api.md) | `open/crates/superi-api` | Transport-neutral public schema catalog, stateless API and project version negotiation, host-injected permission boundary, current state and control facades, bounded deterministic local scripting, bounded ordered event delivery, generated TypeScript contracts, and durable local project hosting | Partial: deterministic versioned discovery, strict API and project compatibility negotiation, JSON-RPC data contracts, fail-closed authorization, current public DTOs and controls, digest-bound `superi-json` interpretation, bounded replay with explicit reconnect recovery, deterministic TypeScript output, and an API-owned local host for no-clobber create, open, mutation, copy, backup, recovery, validation, render settings, and narrow JSON-RPC automation are implemented; authentication, network transport, general dynamic routing, push delivery, persisted replay, public job submission and typed results, general-purpose language evaluation, and full-catalog automation remain absent |
...
| `superi-cli` | [module map](modules/superi-cli.md) | `open/crates/superi-cli` | Headless durable project, media, timeline, render settings, inspect, validate, recovery, JSON-RPC automation, public schema, canonical scenario, and engine validation consumer | Implemented no-clobber creation, complete open and inspect, revision-fenced durable mutation, copy, backup, recovery, render configuration, strict policy input, bounded path-redacted errors, per-request flushed JSON-RPC automation, deterministic `api schema` including permission and version-negotiation metadata, exact fixture scenario transactions, portable expectation verification, eight instrumented contract stages, and strict `engine validate`; version-negotiation command execution, public job hosting, event polling, render submission, muxed publication, persisted history journals, and live application attachment remain absent |
...
| `superi-engine` | [module map](modules/superi-engine.md) | `open/crates/superi-engine` | Open subsystem assembly and orchestration | Partial: engine-wide typed command dispatch, scoped standalone EngineControl hosting, bounded revision-fenced project history, curated behavior-free public editor, project compatibility, and save construction seams, deterministic semantic project diagnostics and coherent editor-state inspection, compound timeline, graph, media, authored audio, extension, and root transactions, undo and redo, settings, audio automation, crash recovery, lifecycle, validation, playback and export dispatch, project preparation, autosave consumer proof, render-export orchestration, arbitration, OpenFX containment, and native audio plugin lifecycle are implemented; production autosave hosting, persisted automation and history journals, live subsystem reconfiguration, concrete lifecycle workers and platform callbacks, automatic arbiter binding, decoded source and prepared-audio binding, native GPU presentation, muxing and publication, concrete plugin transport, public job submission and typed result access, persistent job recovery, and production plugin factories remain absent |
...
2. One explicit registration list builds schema references for all 16 commands, 12 queries,
   eight events, and 11 resources, including discovery method `superi.api.schema.get` itself.
3. `PublicApiSchemaSnapshot` fixes catalog schema `1.3.0`, stable primitive revision 1, and JSON-RPC
...
1. `PublicApiEvent` is a closed union with exactly the eight cataloged event payloads. Publication
...
4. One poll is capped by caller and configured batch bounds. A future cursor fails. An evicted
   ...
   reset barrier plus all ten authoritative state resources and their exact query or typed inspect
   command paths.
...
5. `EngineCommandDispatcher` can attach exactly one project-history owner. Typed history execution
   ...
   publish a correlated `ProjectStateChanged` event. Extension commands retain their typed project
   result through that same replacement event, while settings changes publish a correlated
   `ProjectSettingsChanged` event after the same event-capacity preflight. No-ops publish none.
6. A compound transaction applies one to 64 ordered root, timeline, graph, media, extension, or
   authored audio actions inside one outer project edit. Timeline actions reconcile retained graphs
   through a three-way compile, every action validates the draft, and a late failure rolls back
   everything.
7. History stacks, capacity, and command metadata are session-local operational state. Existing
   ...
   records. The generic API adapter, bounded local scripting consumer, and durable local CLI
   execution are implemented, while persisted history journals, network hosting, full-catalog
   automation, and logging remain later work. Complete editor replacement state is exposed
   separately through the same public facade.
...
1. `superi-api::ProjectEditorApi` owns one dispatcher with an attached authoritative project
   history and exposes permanent schema `1.0.0` command `superi.project.command.execute`.
2. Strict API-owned DTOs cover apply, inspect, undo, redo, all six compound action groups, all 15
   timeline edits, all eight graph mutations, all three media mutations, all four clip-mix
   mutations, and all six extension mutations. Unknown fields and operation tags fail closed.
3. The API converts every identifier, exact time, material, graph value, media path, clip control,
   and extension record before dispatch. Engine exposes only a curated checked construction seam,
...
3. Creation uses production constructors plus an in-memory database and no-clobber save-as. Copy,
   backup, and recovery reuse existing project commands and active-database ownership. Validation is
   a read-only complete current-schema reconstruction.
4. `superi-cli` retains paths as operating-system values and consumes only API DTOs. Media and
...
5. Automation accepts one strict JSON-RPC `2.0` request per bounded JSONL line, echoes string or
...
6. Session undo and redo stacks still do not enter SQLite. The CLI controls selected durable state
   but does not claim a persisted command journal, render submission, container muxing, or network
   server.
...
1. `ProjectDatabase::create` reserves a new path without overwriting, or `memory` creates an
   equivalent in-memory database. Both establish SQLite application ID `SUPR`, schema revision 4,
   semantic format `superi.project` version `1.3.0`, and exactly six strict tables. File-backed
...
2. Preparation serializes the editorial owner through the canonical timeline codec, the validated
   ...
   their stable compound identity and ordered evidence.
...
4. `ProjectDatabase::execute_save_command` builds one complete schema-4 SQLite candidate in the
...
9. Timeline owns `superi.timeline` meaning and strict `TimelineGraphValue` Serde, graph owns
   ...
   revisions, and project-relative meaning survive reload and save-as.
10. `ProjectDatabase::open` validates current schema 4 without mutation or migrates exact schema 0,
   1, 2, or 3 through the contiguous 0-to-1-to-2-to-3-to-4 registry. Schema 0 first reconstructs
   and writes
   ...
   schema 3, and schema 3 adds an empty extension set before writing schema 4 with exact snapshot
   equality, all inside one immediate transaction.
...
12. History stacks remain session-local, while the API-owned local host and CLI now compose public
   database open, no-clobber create, save-copy, backup, validation, recovery, and durable mutation
   workflows over this authority. The API-local script runtime consumes immutable snapshots and
   existing commands without adding a project-owned interpreter. Recovery discovery and restoration
   compose this exact database authority through project, engine, and API owners.
...
6. Focused engine and API contracts prove coherent revisions, strict deterministic round trips,
   explicit detached, unobserved, pending, and observed owners, multichannel routing, muted channels,
   and a sample-exact continuity gap without hidden polling.
...
9. `superi-project` retains clip-mix state in every immutable snapshot and schema-4 audio
...
4. Independent editor, script, and headless-style tests load the document, observe equal snapshots,
   ...
   documents, including strict timeline-owned graph values, in schema-4 SQLite rows, canonicalizes
...
publishes one deterministic schema `1.3.0` catalog for all current commands, queries, events,
...
- The closed public event union matches all eight catalog events exactly. Command events retain
...
The public schema contracts prove the exact 16-command, 12-query, eight-event, and 11-resource
catalog at schema `1.3.0`, current method and resource versions, primitive revision 1, canonical
...
the complete permission vocabulary, and exact metadata for all 28 methods. The version negotiation
...
traces, and durable project agreement. The CLI process contract
...
engine introspection, and asynchronous job lifecycle events all pass through the same broker.
...
edits, caller-owned drafts, schema-4 SQLite, atomic file publication, autosave, engine history,
...
`superi-project` stores canonical current timeline, settings, graph, audio, and extension component
documents inside stable SQLite schema 4. Its exact schema-0, schema-1, schema-2, and schema-3
...
extension set at the 3-to-4 step, and write canonical current rows before commit. One
...
extension records, stable schema-4 SQLite serialization, exact schema-0, schema-1, schema-2, and
schema-3 migration, real engine settings and resource consumers, stable public settings, and
...
Persisted command logs and transport-catalog database adaptation remain absent. The API-owned local
host and CLI now compose the existing project database and file commands, while the project crate
...
- `superi-timeline`: broader OTIO schema and vendor-effect interpretation beyond its pinned 0.18.1
  ...
  edit orchestration, multicam playback and mixing,
...
- Coherent whole-project snapshots, authored clip-mix and opaque extension state, deterministic
  versioned semantic hashing and ordered component diagnostics, schema-4 SQLite storage, supported
  ...
  findings, and exact durable dismissal: `superi-project`.
...
- Native editorial objects, typed track semantics, exact timing and clip retiming, selection, track
  ...
  `superi-timeline`.
- Durable project settings, defaults, strict candidate validation, schema-4 persistence, and
