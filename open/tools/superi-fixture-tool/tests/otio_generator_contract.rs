...
use superi_fixture_tool::{
    generate_otio_baseline, OTIO_BASELINE_TIMELINE_COUNT, OTIO_CANONICAL_SLICE_NAME,
    OTIO_COVERAGE_NAME, OTIO_EXPECTATIONS_NAME, OTIO_MANIFEST_NAME,
};
...
impl TemporaryOutput {
    fn new(label: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "superi-otio-fixture-{label}-{}-{suffix}",
            std::process::id()
        ));
...
#[test]
fn generator_reproduces_canonical_otio_artifacts_byte_for_byte() {
    let first = TemporaryOutput::new("first");
    let second = TemporaryOutput::new("second");

    let first_report = generate_otio_baseline(first.path()).expect("generation must succeed");
    let second_report = generate_otio_baseline(second.path()).expect("generation must repeat");

    assert_eq!(first_report.timeline_count(), OTIO_BASELINE_TIMELINE_COUNT);
    assert!(first_report.payload_bytes() > 0);
    assert!(first_report.payload_bytes() < 128 * 1024);
    assert_eq!(first_report, second_report);

    for name in [
        OTIO_CANONICAL_SLICE_NAME,
        OTIO_COVERAGE_NAME,
        OTIO_EXPECTATIONS_NAME,
        OTIO_MANIFEST_NAME,
    ] {
...
fn generator_refuses_to_replace_an_existing_output_directory() {
    ...
    let error = generate_otio_baseline(output.path()).expect_err("existing output must fail");
