...
use superi_fixture_tool::{
    generate_timing_baseline, TIMING_BASELINE_CASE_COUNT, TIMING_BASELINE_SAMPLE_COUNT,
    TIMING_CATALOG_NAME, TIMING_MANIFEST_NAME,
};
...
impl TemporaryOutput {
    fn new(label: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "superi-timing-fixture-{label}-{}-{suffix}",
            std::process::id()
        ));
...
#[test]
fn generator_reproduces_canonical_timing_artifacts_byte_for_byte() {
    let first = TemporaryOutput::new("first");
    let second = TemporaryOutput::new("second");

    let first_report = generate_timing_baseline(first.path()).expect("generation must succeed");
    let second_report = generate_timing_baseline(second.path()).expect("generation must repeat");

    assert_eq!(first_report.case_count(), TIMING_BASELINE_CASE_COUNT);
    assert_eq!(first_report.sample_count(), TIMING_BASELINE_SAMPLE_COUNT);
    assert!(first_report.catalog_bytes() > 0);
    assert!(first_report.catalog_bytes() < 16 * 1024);
    assert_eq!(first_report, second_report);

    for name in [TIMING_CATALOG_NAME, TIMING_MANIFEST_NAME] {
...
fn generator_refuses_to_replace_an_existing_output_directory() {
    ...
    let error = generate_timing_baseline(output.path()).expect_err("existing output must fail");
