...
/// One native event topic carries every generated public API event.
...
/// Compact transport failure that preserves the complete generated public error.
pub type DesktopTransportError = Box<PublicApiError>;

/// Result type used by the native transport boundary.
pub type DesktopTransportResult<T> = Result<T, DesktopTransportError>;
...
/// Ordered event envelope delivered through the native event topic.
...
impl DesktopTransportOutcome {
    ...
    pub fn into_targeted_parts(self) -> (DesktopTransportReply, Vec<DesktopTransportEmission>) {
...
/// One ordered event projected for one connected client generation.
...
impl DesktopTransportEmission {
    ...
    pub const fn envelope(&self) -> &DesktopEventEnvelope {
        &self.envelope
    }
}
...
/// Shared bounded transport state above the transport-neutral engine handle.
#[derive(Clone)]
...
impl DesktopTransportState {
    ...
    ) -> DesktopTransportResult<DesktopTransportReply> {
        self.dispatch_control_for("main", command)
    }

    /// Executes one control command for the invoking editor client.
    ...
    pub fn dispatch_blocking(
        ...
        command: DesktopTransportCommand,
    ) -> DesktopTransportResult<DesktopTransportOutcome> {
        self.dispatch_blocking_for("main", engine, projects, command)
    }

    /// Executes one command for the invoking editor client on a blocking-safe worker.
    ...
    /// Removes a closing editor client and cancels only its pending work.
    pub fn disconnect_client(&self, client_id: &str) -> DesktopTransportResult<()> {
        validate_client_id(client_id)?;
    ...
    fn admit_request(
        ...
        request_id: &str,
        method: &str,
    ) -> DesktopTransportResult<Arc<AtomicBool>> {
    ...
    fn publish_event(
        ...
        event_name: String,
        payload: Value,
    ) -> DesktopTransportResult<Vec<DesktopTransportEmission>> {
    ...
    ) -> DesktopTransportResult<MutexGuard<'_, DesktopTransportInner>> {
...
fn validate_client_id(client_id: &str) -> DesktopTransportResult<()> {
...
pub fn transport_task_error(operation: &'static str) -> DesktopTransportError {
    transport_error(
        ErrorCategory::Internal,
        Recoverability::Terminal,
...
pub fn event_emission_error() -> DesktopTransportError {
    transport_error(
        ErrorCategory::Unavailable,
        Recoverability::Retryable,
...
mod tests {
    ...
    #[test]
    fn clients_keep_independent_generations_and_receive_one_ordered_event() {
        let transport = DesktopTransportState::new();
