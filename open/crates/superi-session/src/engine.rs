...
use crate::process_runtime::{
    DesktopProcessRuntime, DesktopProcessServiceCounts, DesktopProcessServiceId,
    DesktopProcessServicePhase,
};
...
/// Cloneable, transport-neutral handle to the application engine process.
#[derive(Clone)]
...
/// Explicit owner of one managed EngineControl thread for the application process.
...
impl LinkedEngineProcess {
    ...
    /// Links the dispatcher while reporting every retained execution owner to the session runtime.
    pub fn launch_with_runtime(
        ...
        let pool_config =
            WorkerPoolConfig::recommended(PLAYBACK_WORK_QUEUE_CAPACITY).inspect_err(|error| {
                runtime.update_service(
                    DesktopProcessServiceId::BackgroundWorkers,
        ...
        let worker_pool = Arc::new(BoundedWorkerPool::new(pool_config).inspect_err(|error| {
            runtime.update_service(
                DesktopProcessServiceId::BackgroundWorkers,
                DesktopProcessServicePhase::Failed,
                DesktopProcessServiceCounts::new(0, 0),
        ...
        runtime.update_service(
            ...
            DesktopProcessServiceCounts::new(worker_count, worker_count),
            true,
            worker_names.clone(),
...
fn run_engine_control(
    ...
    loop {
        ...
        match requests.recv_timeout(CONTROL_WAIT) {
            Ok(request) => {
                execute_request(request, dispatcher.as_ref(), &mut editor_session, &playback)
            }
...
impl ProjectEditorSession {
    fn load(route: &DesktopProjectEditorRoute, playback: &PlaybackOwnerConnection) -> Result<Self> {
...
fn project_playback_bounds(
    ...
    let duration =
        superi_engine::editor::Duration::new(duration.value().max(1), timeline.edit_rate())?;
...
mod tests {
    ...
    fn retained_editor_session_persists_apply_undo_redo_and_exact_events() {
        let worker_pool =
            Arc::new(BoundedWorkerPool::new(WorkerPoolConfig::new(2, 32).unwrap()).unwrap());
        let (playback_requests, playback_receiver) = sync_channel(PLAYBACK_CONTROL_CAPACITY);
        let playback = PlaybackOwnerConnection {
            requests: playback_requests,
        };
