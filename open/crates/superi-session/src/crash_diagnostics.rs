//! Session-local crash diagnostics and cross-session recovery intent.
//!
//! This owner retains one bounded private journal and one active-session marker under the
//! application data directory. Public snapshots contain reviewed recovery guidance and stable
//! context only. Raw panic detail remains local and never crosses the public boundary.
