//! Explicit desktop process ownership and ordered shell-task cleanup.
...
/// Stable desktop process phases exposed to the shell.
...
impl DesktopProcessSnapshot {
    ...
    /// Returns whether new shell background tasks may be admitted.
    #[must_use]
...
type ShellTaskHandle = async_runtime::JoinHandle<ShellTaskOutcome>;
...
/// Shared shell-local owner registry for the complete desktop process.
#[derive(Clone)]
...
impl DesktopProcessRuntime {
    ...
    /// Closes shell task admission before ordered owner teardown begins.
    ...
    /// Admits one bounded blocking shell task and retains its join handle.
        ...
        F: FnOnce() + Send + 'static,
    {
        ...
        let handle = async_runtime::spawn_blocking(move || {
            let outcome = match start_receiver.recv() {
                ...
                Err(_) => Err("project file association task admission was interrupted".to_owned()),
            };
    ...
    /// Joins every admitted blocking shell task, even after an earlier failure.
    ...
    pub(crate) fn update_service(
        ...
        service.phase = phase;
        service.owned_units = owned_units;
        service.active_units = active_units.min(owned_units);
    ...
    fn reap_finished_background_tasks(&self) -> Result<()> {
        let finished = {
            ...
            let (finished, pending): (Vec<_>, Vec<_>) = handles
                .into_iter()
                .partition(|handle| handle.inner().is_finished());
