...
test("the production timeline mounts one accessible retime editor on the generic command route", () => {
  const workspace = readFileSync(
    resolve(appRoot, "src/timeline-workspace.tsx"),
    "utf8",
  );
  const editor = readFileSync(
    resolve(appRoot, "src/timeline-retime-editor.tsx"),
    "utf8",
  );
  ...
  assert.match(workspace, /<TimelineRetimeEditor/);
  assert.match(workspace, /onExecuteProjectCommand\(plan\.request\)/);
  assert.match(workspace, /commandPendingRef\.current/);
  assert.match(workspace, /executeHistory\("undo"\)/);
  assert.doesNotMatch(editor, /@tauri-apps|invoke\(/);
  for (const evidence of [
    ...
    assert.ok(editor.includes(evidence), `missing retime evidence: ${evidence}`);
  }
  assert.match(styles, /\.timeline-retime-curve polyline/);
  assert.match(packageJson, /tests\/timeline-retime\.test\.ts/);
});
